...
OOS_REL_N_MONTHS = 2
STAGES = ['S0', 'S1', 'S2', 'S3', 'S3B', 'S4', 'S5', 'S6', 'S5B', 'S7', 'S8', 'S8B', 'S9']
FAMILIES = [
...
def s5b_selection(df, ad, st, w, pool, anchor, book_file, out, input_sha, attest):
    ...
    tdays = int(pd.Series(bk['exit_time'].values).str[:10].nunique())
    ...
    _write_with_header(os.path.join(out, 'selection_depthyield_grid.csv'), grid, [
        ...
        f'dataset_rows={attest["rows"]} segment={segment_label} traded_days={tdays}',
...
def main():
    ...
    contenders = committed = profile = evidence = selection_state = None
