...
constraints). Pearson FailCorr is REPORTED ONLY. They are never combined into a
composite score anywhere in this file; the objective is lexicographic, evaluated
step by step, and each step returns its own value.
...
diminishing-returns violations so the decision rests on measurement.
"""
...
def depth_yield_direction(entry_bars_d, n_signals_d, traded_day_count, S, n_tol):
    if n_signals_d <= 0 or traded_day_count <= 0:
        return 0.0, 0
    sizes = clusters_from_entries(entry_bars_d, n_tol)
    ge = int(sum(1 for s in sizes if s >= S))
    return float(ge) / float(traded_day_count) / float(n_signals_d), ge
...
def greedy_direction(direction, candidate_ids, gain_fn, constraint_fn,
                     eps=MARGINAL_GAIN_EPS, max_steps=None):
    selected = []
    log = []
    heap = []
    ...
    while heap:
        ...
        cid = top[1]
        if gain <= eps:
            stop_reason = f'marginal gain {gain:.3e} <= eps {eps:.3e}'
            break
        ...
        step += 1
    return selected, stop_reason, pd.DataFrame(log)
